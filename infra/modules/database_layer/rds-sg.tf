...
resource "aws_security_group" "instance" {
  name        = "${var.cluster}-db-access"
  description = "Used in ${var.workspace} to enable RDS access"
  vpc_id      = "${var.vpc_id}"
...
resource "aws_security_group" "rds" {
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = ["${aws_security_group.instance.id}", "${var.jump_ssh_sg_id}", "${aws_security_group.lambda_sg.id}"]
  }
